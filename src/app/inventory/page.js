...
import InventoryService from "@/services/warehouse/inventory/inventory"
...
export default class Inventory extends React.Component{
    ...
    addInventory = (data) => {
        this.request.addInventory(data)
            .then(ok => {
                this.getInventory(); // Refresca la lista de empleados
    ...
    updateInventory = (id, newData) => {
        const oldData = this.state.inventory.find(emp => emp.id === id);
        const updatedData = { ...oldData, ...newData }; // Fusiona los datos antiguos con los nuevos
        this.request.updateInventory(id, updatedData)
            .then(ok => {
                this.getInventory(); // Refresca la lista de empleados
    ...
    deleteInventory = (id) => {
        this.request.deleteInventory(id)
            .then(ok => {
                this.getInventory(); // Refresca la lista de empleados
    ...
    render(){
        const { showColumnLines, showRowLines, rowAlternationEnabled, inventory, focusedRowKey } = this.state;
        return(
            <div>
                <React.Fragment>
                <DataGrid
                            ...
                            if (e.changes && e.changes.length > 0) {
                                ...
                                if (confirm) {
                                    ...
                                    if (key && typeof key === "string" && key.startsWith("_DX_KEY_")) {
                                        // Es una inserción
                                        this.addInventory(cambios);
                                    } else {
                                        // Es una actualización
                                        const id = key;
                                        this.updateInventory(id, cambios);
                                    }
                                } else {
                        ...
                        onRowRemoving={async (e) => {
                            ...
                            this.deleteInventory(e.data.id);
                        }}
                    >
                        ...
                        <Editing
                            ...
                            allowUpdating={true}
                            allowDeleting={true}
                            onRowInserting={(e) => this.addInventory(e.data)}
                            onRowUpdating={(e) => this.updateInventory(e.key, e.newData)}
                        >
                            <Popup title="Inventory Info" showTitle={true} width={700} height={525} />
                            <Form>
